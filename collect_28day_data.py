...
import floodsystem.config as config
...
def collect_live_data(duration_days=28):
    ...
    # Save collected data
    print(f"\nSaving {len(collected_data['stations'])} stations to {config.LIVE_DATA_FILE}...")
...
def generate_mock_data(num_stations=50, duration_days=28):
    ...
    print(f"Saving to {config.MOCK_DATA_FILE}...")
    with open(config.MOCK_DATA_FILE, 'w') as f:
...
def load_data(use_live_data=None):
    ...
    data_file = config.LIVE_DATA_FILE if use_live_data else config.MOCK_DATA_FILE
...
def run():
    ...
    print("║" + f"  Mode: {'LIVE DATA' if config.USE_LIVE_DATA else 'MOCK DATA'}".center(68) + "║")
    print("║" + " " * 68 + "║")
    print("╚" + "═" * 68 + "╝")
    print()
    
    # Get data (live or mock based on configuration)
    data = get_data(use_live_data=config.USE_LIVE_DATA)
    
    # Print summary
    print_data_summary(data)
    
    print("\n" + "=" * 70)
    print("To change between live and mock data, set floodsystem.config.USE_LIVE_DATA")
    print("variable in any task or script (currently set to {})".format(config.USE_LIVE_DATA))
    print("=" * 70 + "\n")
